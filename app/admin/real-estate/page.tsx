...
import { Home, Search, Download, TrendingUp, TrendingDown, DollarSign, Users, MapPin } from "lucide-react"
...
export default function AdminRealEstatePage() {
  ...
  const loadInvestments = async () => {
    try {
      ...
      if (error) {
        console.error("Error loading real estate investments:", error)
        toast({
          title: "Error",
          description: "Failed to load real estate investments.",
          variant: "destructive",
        })
        return
      }
      ...
      const transformedInvestments = (investmentsData || []).map((inv: any) => ({
        ...
        propertyValue: Number(inv.property_value),
        investmentPercentage: inv.investment_percentage ? Number(inv.investment_percentage) : null,
        expectedReturn: inv.expected_return ? Number(inv.expected_return) : null,
        currentValue: inv.current_value ? Number(inv.current_value) : null,
        currentInvestmentValue: inv.current_investment_value ? Number(inv.current_investment_value) : null,
        ...
        propertyImageUrl: inv.property_image_url,
        bedrooms: inv.bedrooms,
        bathrooms: inv.bathrooms,
        squareFeet: inv.square_feet,
        yearBuilt: inv.year_built,
        rentalIncome: inv.rental_income ? Number(inv.rental_income) : null,
        maintenanceCost: inv.maintenance_cost ? Number(inv.maintenance_cost) : null,
      ...
      const totalValue = transformedInvestments
        ...
        .reduce((sum: number, inv: any) => sum + (inv.currentInvestmentValue || inv.amountInvested || 0), 0)
  ...
  const filteredInvestments = investments.filter((investment) => {
    const matchesSearch =
      ...
      investment.city?.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            <div className="px-6 py-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-500">Monitor real estate investments and property performance</p>
                </div>
          ...
          <main className="p-6">
            ...
            <div className="bg-white rounded-lg shadow">
              <div className="px-6 py-4 border-b">
                <h3 className="text-lg font-semibold text-gray-900">Real Estate Investments</h3>
              </div>
                      ...
                      filteredInvestments.map((investment) => (
                        <tr key={investment.id}>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm font-medium text-gray-900">{investment.propertyName}</div>
                            <div className="text-xs text-gray-500">{investment.propertyType || "N/A"}</div>
                            {investment.bedrooms && investment.bathrooms && (
                              <div className="text-xs text-gray-400">
                                {investment.bedrooms}bd, {investment.bathrooms}ba
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            {investment.investmentPercentage && (
                              <div className="text-xs text-gray-500">
                                {investment.investmentPercentage.toFixed(2)}% ownership
                              </div>
                            )}
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            {investment.currentInvestmentValue ? (
                              <div className="text-sm font-bold text-gray-900">
                                ${investment.currentInvestmentValue.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </div>
                            ) : (
